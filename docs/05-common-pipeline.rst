...
The fixed-score packages and the generative packages share the same general output pipeline. They differ mainly in how the musical material is created. ``modus_operandi_abjad`` and parts of ``jazz_rhythm`` are more direct and declarative. The quartet packages are more explicitly generative. They load configuration, derive generation constraints, create event streams on a quantized grid, and then convert those events into notation.
...
The main quartet CLI shows this pattern clearly. The key point is not the argument parser itself. It is the branch at the end that decides whether to compile notation only, render a single SoundFont path, or render separate piano and string layers:
